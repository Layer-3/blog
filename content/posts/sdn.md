...
author = ["tang"]
date = 2020-03-22
lastmod = 2020-04-13T16:29:07+08:00
tags = ["SDN", "Network"]
categories = ["Study"]
draft = false
+++
...
值得注意的是，文章提到了一个关键的网络基础结构：[Merlin](http://frenetic-lang.org/merlin/merlin-hotnets2013.pdf)，其是一个用于控制不同网络组件（例如转发设备，中间盒和终端主机）的统一框架。Merlin 为每种类型的组件生成特定的代码。 以策略定义为输入，Merlin 的编译器确定转发路径，转换位置和带宽分配。 编译后的输出是要安装在设备中的针对不同组件的低级指令集。 Merlin 的策略语言还允许运营商将子网的控制权委派给租户，同时确保隔离。 这种委托控制是通过策略来表达的，每个租户所有者可以进一步细化这些策略，从而使他们可以根据自己的特定需求自定义策略。
